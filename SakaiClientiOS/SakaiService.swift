...
class SakaiService {
    ...
    func getAllGrades(completion: @escaping (_ grades: [[[GradeItem]]]?) -> Void) {
        ...
        RequestManager.shared.makeRequest(url: url, method: .get) { res in
            ...
            for site in collection {
                // Every object within the collection has general site data with an "assignments" array that
                // represents the gradebook
    ...
    func getSiteGrades(siteId: String, completion: @escaping (_ grades: [GradeItem]?) -> Void) {
        ...
        RequestManager.shared.makeRequest(url: url, method: .get) { res in
            ...
            //Ensure the JSON data has an "assignments" array
    ...
    func getSiteAssignments(for siteId: String, completion: @escaping (_ assignments: [Assignment]?) -> Void) {
        ...
        RequestManager.shared.makeRequest(url: url, method: .get) { res in
            ...
            guard let assignmentsJSON = JSON(data)["assignment_collection"].array else {
                //Ensure the JSON data has an "assignments" array
                completion(nil)
                return
            }
    ...
    func getAllAnnouncements(offset: Int,
                             limit: Int,
                             daysBack: Int,
                             completion: @escaping (_ announcements: [Announcement]?, _ moreLoads: Bool) -> Void,
                             siteId: String? = nil) {
        var url: String
        if let siteId = siteId {
            url = AppGlobals.SITE_ANNOUNCEMENTS_URL.replacingOccurrences(of: "*", with: siteId)
                .replacingOccurrences(of: "#", with: "\(limit)")
        } else {
