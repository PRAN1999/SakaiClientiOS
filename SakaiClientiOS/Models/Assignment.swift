...
struct Assignment: TermSortable, SiteSortable {
    ...
    ///   - instructions: The description for the Assignment and any instructions the instructor may have included. May
    ///     include links and may include HTML tags and markdown
    ///   - attributedInstructions: The instructions for the Assignment after being parsed as HTML, to properly
    ///     format links and other HTML tags
    ...
    ///   - attachments: A List of attachments for the assignment, represented as NSAttributedString with embedded links
    ///     to the content
    ///   - siteURL: A String containing the URL for the Assignment entity page, where Assignment submission can occur
    ///     securely
    ...
    /// Uses a JSON object returned from network request and parses it to instantiate an Assignment object with all
    /// available fields
    ///
    /// - Parameter data: A JSON object representing a Sakai Assignment, which contains all relevant information for the
    ///     assignment
    init(data: JSON) {
        ...
        self.init(title, dueTimeString, dueDate, term, siteId, siteTitle, instructions, attributedInstructions,
                  status, maxPoints, currentGrade, resubmissionAllowed, attachmentStrings, siteUrl)
    }
}
